...
static void drawKaleidoscope(Canvas& c) {
    ...
    for (int i = 0; i < petals; ++i) {
        c.push();
        c.rotate(state.t * 0.4 + i * 2 * kPi / petals);   // 转起来
        c.translate({20, 0});                          // 挪到花心外
        c.scale(0.7 + 0.3 * std::sin(state.t * 2 + i));     // 一呼一吸地缩放
        c.fill(Color::hsv(i * 360.0 / petals + state.t * 40, 0.75, 0.95, 0.85));
        c.noStroke();
        c.beginShape();
...
static void seedTrailDemo() {
    ...
    for (int i = 1; i <= 60; ++i) {
        ...
        state.ink.stroke(Color::hsv(i * 6.0, 0.8, 0.95), 3).line(prev, cur);
        prev = cur;
    }
}
...
static void updateTrail(App& app) {
    ...
    if (!c.hovered()) { state.hasLast = false; return; }
    Vec2 m = c.mouse();
    if (state.hasLast) {
        state.ink.stroke(Color::hsv(std::fmod(state.t * 80, 360.0), 0.8, 0.95), 3).line(state.last, m);
    }
    state.last = m;
    state.hasLast = true;
}
...
static void drawTerrain(Canvas& c) {
    ...
    for (double x = 0; x <= 100; x += 2)
        pts.push_back({x, remap(noise(x * 0.05, state.t * 0.3), 0, 1, 20, -20)});
...
static void drawSpriteAudio(Canvas& c) {
    ...
    if (!state.sprite) return;                                // 没加载出来就什么都不画，别崩
    int    frame = (int)std::fmod(state.t * 6.0, 4.0);
    double lv = audio::loudness();                         // 放出去的响度，Scratch 的「响度」
    double size = 32 * (1.0 + lv * 0.6);                    // 越响精灵越大
    c.image(state.sprite, Rect::fromCenter({0, -6}, size, size), Rect(frame * 32.0, 0, 32, 32));
...
static void seedFlowField() {
    state.flowPts.resize(State::kFlowParticles);
    for (Vec2& p : state.flowPts) p = {rng().d(0, State::kFlowW), rng().d(0, State::kFlowH)};
}
static void updateFlowField() {
    if (!state.flow.valid()) return;
    Canvas& c = state.flow.begin();          // 之后画的东西都进 flow 这块缓冲（像素坐标）
    const double noiseScale = 0.004;     // 噪声采样密度：越小，流线越"顺"、转弯越慢
    const double stepLen = 2.2;          // 每帧往前爬这么多像素
    for (Vec2& p : state.flowPts) {
        double angle = noise(p.x * noiseScale, p.y * noiseScale, state.t * 0.06) * 2 * kPi * 2;
    ...
    state.flow.end();                        // 这一批线这时才真正被烧进纹理
}
static void drawFlowField(Canvas& c) {
    c.camera().fit(Rect(0, 0, State::kFlowW, State::kFlowH));
    c.image(state.flow.texture(), Rect(0, 0, State::kFlowW, State::kFlowH));   // 整块贴出来，O(1)
}

int main(int argc, char** argv) {
    App app(argc, argv);
    app.title("创意包合集").size(1180, 760).theme(Theme::Paper());
    state.part = (int)cli::args().num("part", 0);   // --part 0..4：跳过面板直接看某一节（截图用）
    state.ink.limit(3000);

    app.onStart([] {
        state.sprite = loadTexture("assets/sprite.png", true);   // 只能在这里加载贴图；像素风精灵传 true，放大不糊
        seedTrailDemo();
        // Graphics::create 也只能在这儿（或 onFrame 里）调——要先有显卡上下文。
        if (state.flow.create(State::kFlowW, State::kFlowH)) seedFlowField();
    });

    app.onFrame([&app](double dt) {
        state.t += dt;
        if (state.part == 1) updateTrail(app);
        if (state.part == 4) updateFlowField();   // 每帧往 flow 里烧几千条短线，O(1)，不重放
    });

    app.onDraw([](Canvas& c) {
        switch (state.part) {
            case 0: drawKaleidoscope(c); break;
            case 1: c.draw(state.ink); break;   // 每帧把记下来的笔迹重放一遍
    ...
    app.onClick([](Vec2, Mouse) {
        if (state.part == 3) audio::play("assets/beep.wav");   // Scratch 的「播放音效」
    });

    app.onPanel([&app] {
        if (ui::section("小节")) {
            ui::select("切换", &state.part, {"万花筒", "拖尾", "噪声地形", "精灵动画 + 声音", "流场"});
            if (state.part == 1 && ui::button("清空尾巴", true)) state.ink.clear();
            if (state.part == 2 && ui::button("换一片地形", true)) noiseSeed(++state.seedVal);
            if (state.part == 3) ui::help("点一下画布放个音效，柱子是频谱，精灵会随响度放大。");
            if (state.part == 4) {
                ui::help("离屏画布（Graphics）：几千条短线画进一块 GPU 位图，永远不清，几十万条也不掉帧。");
                if (ui::button("清空", true)) state.flow.clear();
            }
        }
