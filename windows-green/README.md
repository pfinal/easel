...
**64 位 Windows 10（1803）或更新。** 解压用的是系统自带的 `tar.exe`，1803 起才有。

除此之外机器上什么都不用装 —— 尤其是不用单独装 C++，编译器就在包里。

> 显卡驱动不用操心：装完 Windows、Windows Update 跑过一遍，厂商驱动和 OpenGL 3.2 就都有了。
> 只有几种边角情况会掉回「Microsoft 基本显示适配器」（OpenGL 只有 1.1，窗口开不起来）——
> 纯净镜像 + 断网 + Windows Update 被禁；远程桌面里跑；虚拟机没开 3D 加速；
> 2010 年以前的核显。真碰上了，`app --doctor` 会把显卡和后端直接打出来，一句话定位；
> 退路是 DX11 后端（`cmake --preset dx11`，代码写了没实测），
> 或者往 exe 旁边放一份 Mesa 的软件渲染 `opengl32.dll`。
