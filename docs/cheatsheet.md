...
上面这段 `main` 编出来的可执行文件（`app.exe` / `app`，点「运行」时跑起来的那个子进程）
认这些参数；工作台自己的命令表（`--new` / `--build` / `--run` / `--package` / `--export` / `--clean` / `--doctor`）
...
窗口分四段，从上到下就是「哪个工程 → 点哪里 → 怎么样了 → 凭什么」：菜单栏、工程栏、
运行 / 停止、结论那一行，最下面是输出区。界面规则写在 [`ui-design.md`](ui-design.md) 里。

| 在哪 | 干什么 |
|---|---|
| **运行**（F5） | 编译 + 把作品跑起来（**作品是子进程，自己一个窗口**）。报错行点一下就跳到出错的代码。作品正在跑的时候再按一次 = 停掉它、重新编译运行 |
| **停止** | 掐掉正在跑的作品（编译中按它就是中断编译） |
| 结论那一行 | 「完成 编译 2.5 秒 · 8.6 MB」「失败 3 个错误 · 点这里看第一个」。失败时点它直接跳到第一条错误 |
| 文件 → 新建工程… | 骨架下拉默认是**空白**：只有 `src/app.cpp`，画一个 Hello。选「算法骨架」才给 `src/solver.cpp` + 数据文件 + 回放 + 收敛曲线；也可以直接从一个自带示例建。作品名只能是英文 |
| 文件 → 打开工程… / 最近打开 | 换一个工程 |
| 文件 → 导出应用程序… | Release 版 + assets/data + README.txt，放进 `dist/<名字>-release/`，可以直接双击 |
| 文件 → 导出工程… | 自足的完整工程，不装 Easel、不联网也能编 |
| 文件 → 清理 | 删掉这个工程的构建产物（`.easel/build` 和 `.easel/build-release`），腾出空间。**不动 `dist/` 里导出的东西，也不动你的代码**。构建目录坏了、编不动了，先清理再运行 |
| 文件 → 复制工程路径 / 复制输出 / 打开日志文件 | 拿去别处用；完整输出存在 `.easel/last-build.log` |
| 设置 → 运行参数… | 原样传给作品的命令行参数（只对这次打开有效） |
| 设置 → 详细输出 | 多打一截排障细节，等价于命令行的 `--verbose` |
| 设置 → 紧凑模式 | 把输出区收起来，窗口缩成一条（编译失败时会自动长高把错误露出来） |
| 设置 → 始终置顶 | 让工作台压在 VS Code 上面，改完代码不用找窗口 |
| 帮助 → 环境自检 | 把环境情况打进输出区，等价于 `easel --doctor` |
