...
| `easel.exe` | **预编译主程序，从这里开始**：双击直接打开工作台，不用等编译（带了预编译包才有这个文件） | 13 MB |
| `easel.bat` | 备用入口：新建工程 / 运行 / 停止 / 导出应用程序 / 导出工程 / 清理。没有 `easel.exe`（或者源码有改动要重编）时用它，会自己把工作台编出来（第一次几分钟，以后就直接开了） | |
...
> 写代码用 VS Code（或任何编辑器），**运行、停止、导出应用程序、导出工程、清理走工作台**
> （双击 `easel.exe`；没带预编译包时用 `easel.bat`，源码有改动会自己重编，D-29）。
> 作品是工作台的子进程，所以改完界面代码点一下就能重跑。
