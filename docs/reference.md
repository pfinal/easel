...
`frameRate(double fps)`：帧率上限，默认 60。传 `0` 关掉上限，完全交给垂直同步。垂直
同步在虚拟机 / 没装驱动的机器上经常失效，一旦失效又没设上限，主循环会空转烧一个
CPU 核——所以默认给了这个兜底。命令行 `--fps N` 会在这句话之后覆盖这里设的值；
窗口最小化或被完全遮住（`framebuffer` 尺寸变 0）时，不管这里设了多少，都会自动降到
约 10 帧，不会因为忘关而占满一个核。

`idleThrottle(bool)`：工具类程序专用，默认关。开启后，只要这一帧没有任何鼠标 / 键盘
输入，且已经闲置超过 0.5 秒，就把这一帧的间隔放大到 100ms（10 帧）；一有输入、或者
编辑栏的编译/运行子进程还在跑、或者 toast / 错误横幅正显示着，立刻恢复 `frameRate()`
设的速度。工作台这类「界面大部分时间都在发呆」的程序用得上；作品一般不用开——画布
自己的动画不该被这个打断。

`busyWhen(fn)`：开了 `idleThrottle` 时，这个回调返回 `true` 就保持满帧（比如有自己的子
进程在输出）。参数 `fn` 是一个 `std::function<bool()>`。
...
Windows 上作品是 GUI 程序，没有控制台；从命令行运行时输出会接回当前终端。
